
--Views, Encryption, Computed Column based functions, Table level-check constarint

----Computed Columns based on a Function
ALTER TABLE [Hotel]
ADD [final_amount] AS (price*no_of_travellers);

ALTER TABLE [Fare] 
ADD [Total_Fare] AS (amount+(amount*(tax/100))-(amount*discount/100));

----Column Encryption on dbo.traveller - (Phone, Email)
CREATE COLUMN MASTER KEY MyCMK
WITH
(
    KEY_STORE_PROVIDER_NAME = N'MSSQL_CERTIFICATE_STORE',
    KEY_PATH = N'CurrentUser/my/215BE046B1650F033DD1742BA9FCE358E820C342'
)


CREATE COLUMN ENCRYPTION KEY MyCEK1   
WITH VALUES  
(  
    COLUMN_MASTER_KEY =  MyCMK,   
    ALGORITHM = 'RSA_OAEP',   
    ENCRYPTED_VALUE = 
);

------Views
CREATE VIEW [Flight_details1] AS
SELECT *
FROM flight_trip
WHERE No_of_travellers = 2;


CREATE VIEW [hotel_details] AS
SELECT *
FROM hotel
WHERE final_amount > 400;

-----Table-Level Check Constraint
Alter table Fare 
add constraint chk_discount check(discount<25);

Alter table Airplane_Specs 
add constraint chk_seat_capacity check(seating_capacity<400);